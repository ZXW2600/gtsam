...
template <POSE, POINT>
virtual class PoseToPointFactor : gtsam::NoiseModelFactor {
  PoseToPointFactor(gtsam::Key key1,gtsam:: Key key2, const POINT& measured, const gtsam::SharedNoiseModel&  model);
  void print(const std::string& s, const gtsam::KeyFormatter& keyFormatter ) const;
  bool equals(const gtsam::NonlinearFactor& expected, double tol ) const;
  gtsam::NonlinearFactor::shared_ptr clone() const;
  Vector evaluateError(const POSE& w_T_b,
                       const POINT& w_P,
                       gtsam::OptionalMatrixType H1,
                       gtsam::OptionalMatrixType H2) const;
  const POINT& measured() const;
};
typedef gtsam::PoseToPointFactor<gtsam::Pose3, gtsam::Point3> Pose3ToPoint3Factor;
